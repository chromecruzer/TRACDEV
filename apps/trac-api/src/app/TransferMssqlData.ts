...
export default class TransferMssqlData {
  ...
  public transferHttp(postgresClient: PostgresClient) {
    return tryCatch(async (req, res, next) => {
      ...
      const jtype = JSON.stringify(client,null,2)
      await this.transfer(client, user)
        .finally(async () => {
          console.log(`DATA READY TO EMIT ${jtype}`)   // added by me 
      ...
      await this.refresher.refresh(client, user)
        ...
        .finally(async () => {
          console.log("this is sent to  msssql")              // added by me 
          return postgresClient.release(client);
        });
  ...
  public async transfer(pclient, user) {
    ...
    const customerIds = _.filter(await this.iolExtract.getUniqueCustomerCodes(pclient),
        c => _.isString(c));
    ...
    await this.tediousClient.release(request); // mssql disconnect 
