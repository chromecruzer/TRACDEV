...
export class UploadDataTransformer extends UploadTransformerA {
  ...
  async transform(uploadedData: InputDataTypeLabel[][], client, token: string) {
    const json = await this.transformData.transform(this.transformData.convertSpreadsheetRecords(uploadedData), client)
