...
constructor(schemaName: string, private tableName, protected ns = null  as (NotificationsServer | null)) {
  ...
  public async  storeInDb(data, client) {
    ...
    if(!_.isEmpty(adds)) {      // not of empty //true
      if(this.ns) {
  ...
  public convertSpreadsheetRecords(uploadedData: InputDataTypeLabel[][]) {
    if (this.ns) {
      this.ns.notify('INFO', `convert spreadsheet data`,
        `converting ${uploadedData.length} records`, ['iolUpload'])
    }
    // console.info(`convert spreadsheet data ${dump(transformHelpers[this.tableName].mappings)}`);
    return _.map(uploadedData, u => recordFromArray(u,transformHelpers[this.tableName].mappings));
  }
}
