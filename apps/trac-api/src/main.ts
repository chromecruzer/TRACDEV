...
const initialize = async () => {
  ...
  /**
   * @swagger
   * /upload/iol_report/submit:
   *  post:
   *    description: upload an IOL report
   *
   *
   */
  ...
  app.post(`/${prefix}/users/createUser`, userOps.createUser(postgresClient));
  app.post(`/${prefix}/users/updatePwd`, userOps.updatePwd(postgresClient));
